 ...
 *
 */
class CRM_Dataquality_Form_Report_PuActivity extends CRM_Report_Form {
  ...
  protected $_puFieldSet = NULL;

  protected $_puField = NULL;
  protected $_puFieldAction = NULL;
  protected $_puActivityType = NULL;
  protected $_puFieldDescription = Null;
  protected $_PuFieldHow = NULL;
  protected $_PuFieldAutomation = NULL;
  /**
   * Class constructor.
   */
 ...
 //                   'default' => 'this.month',
    ...
    if (isset($this->_puFieldSet) && isset($this->_puField)) {

      $this->_columns[$this->_puFieldSet]["fields"][$this->_puField]["default"] = 1;

    }
    if (isset($this->_puFieldSet) && isset($this->_puFieldDescription)) {

      $this->_columns[$this->_puFieldSet]["fields"][$this->_puFieldDescription]["default"] = 1;
    ...
    if (isset($this->_puFieldSet) && isset($this->_PuFieldAutomation)) {
      ...
      $this->_columns[$this->_puFieldSet]["fields"][$this->_PuFieldAutomation]["default"] = 1;

    }

    foreach($this->activityTypes as $aid => $aname){

      if ($aname == "Pu") {
        $this->_columns['civicrm_activity']['filters']['activity_type_id']['default'] = $aid;
        break;
      }
    ...
    foreach ($this->_columns as $colname => $col){
      if (isset($col["group_title"]) && $col["group_title"] == "PuFields"){
        ...
        if ($col["fields"]) foreach ($col["fields"] as $fieldname => $field) {
          if ($field["title"] == "Pu value")
          {
            $this->_puField = $fieldname;
          }
          elseif ($field["title"] == "Pu Description") $this->_puFieldDescription = $fieldname;
          elseif ($field["title"] == "Pu How") $this->_PuFieldHow = $fieldname;
          elseif ($field["title"] == "Pu Action") $this->_puFieldAction = $fieldname;
          elseif ($field["title"] == "Pu Automation") $this->_PuFieldAutomation = $fieldname;
